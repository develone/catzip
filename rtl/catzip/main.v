...
// DO NOT EDIT THIS FILE!
//
// CmdLine:	autofpga autofpga -o . global.txt bkram.txt buserr.txt clockpll40.txt pic.txt pwrcount.txt version.txt hbconsole.txt gpio.txt dlyarbiter.txt zipbones.txt sdramdev.txt sdramscope.txt
//
...
parameter	RDLY = 6;
	...
	//output wire [15:0] r_ram_data; //did not help adding these
	//output	wire		o_ram_drive_data;
	output	wire		ram_drive_data;
...
//x2	Was a master bus as well
	...
	always @(posedge i_clk)
	begin
		casez({		sdramscope_ack,
				wb_dio_ack,
				console_ack,
				wb_sio_ack,
				bkram_ack	})
			5'b1????: wb_idata <= sdramscope_data;
			5'b01???: wb_idata <= wb_dio_data;
			5'b001??: wb_idata <= console_data;
			5'b0001?: wb_idata <= wb_sio_data;
			5'b00001: wb_idata <= bkram_data;
			default: wb_idata <= sdram_data;
		endcase
	end
	assign	wb_stall =	((sdramscope_sel)&&(sdramscope_stall))
...
`else	// SDRAM_SCOPE
	...
	// In the case that there is no sdramscope peripheral responding on the wb bus
	reg	r_sdramscope_ack;
	initial	r_sdramscope_ack = 1'b0;
	always @(posedge i_clk)	r_sdramscope_ack <= (wb_stb)&&(sdramscope_sel);
	assign	sdramscope_ack   = r_sdramscope_ack;
	assign	sdramscope_stall = 0;
	assign	sdramscope_data  = 0;
